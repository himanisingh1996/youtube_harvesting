############ Project created by Himani Singh ##############
############ Documentation / Comments are created wherever required using # symbol ##############
...
# MongoDB library install if not available using pip install pymongo
import pymongo
...
# Streamlit libraries and plotly for creating charts
...
# Streamlit Design 
st.set_page_config(layout='wide')   ## For wide screen monitors

# Title
st.title(':firebrick[A Project on Youtube Channel Data Harvesting, Data Migration to MySql and Analytics]')

# Code for Data Harvesting starts here
col1, col2 = st.columns(2)
with col1:
    st.header(':violet[Step1: Data Harvesting]')
    st.write ('(Note:- This zone **collect data** by using channel id and **stores it in the :green[MongoDB] database**.)')
    ...
    if Get_data or st.session_state.Get_state:
        ...
        api_version = 'v3'
       # api_key = 'AIzaSyAdEaRLlERcGSsB7-UzGao5c0aWx7ISJas'
        api_key = 'AIzaSyDIKEHterp4-aYMnxte1ihD5wIO-ys8-0A' #Youtube API Key
        ...
        # Function call to Get Channel data from a channel ID 
        channel_data = get_channel_data(youtube,channel_id)
 #       if (channel_data==None):   #Exception handling if channel returns no data
        ...
        # Function Definition to retrieve video IDs from channel playlist
        def get_video_ids(youtube, channel_playlist_id):    #this function extracts Video IDs for entered channel ID
    ...
    if Migrate or st.session_state_migrate_sql:
        ...
        for i in range(1,len(result['Channel_data'])-1):
            video_details_tosql = {
                ...
                'Video_Name': result['Channel_data'][f"Video_Id_{i}"]['Video_Name'].decode(),
                'Video_Description': result['Channel_data'][f"Video_Id_{i}"]['Video_Description'].encode(),
        ...
        mycursor.execute("CREATE DATABASE IF NOT EXISTS youtube_db DEFAULT CHARACTER SET = 'utf8' DEFAULT COLLATE 'utf8_general_ci'")
        mycursor.execute("USE youtube_db")
        mycursor.execute("CREATE TABLE IF NOT EXISTS comments ( Video_Id varchar(225) DEFAULT NULL,  Comment_Id varchar(225) DEFAULT NULL,  Comment_Text blob, Comment_Author varchar(225) DEFAULT NULL,  Comment_Published_date varchar(50) DEFAULT NULL)")
        ...
        engine = create_engine('mysql+mysqlconnector://root:123456@localhost/youtube_db?charset=utf8', echo=False)
        ...
        Comments_df.to_sql('comments', engine, if_exists='append', index=False,
                        dtype = {'Video_Id': sqlalchemy.types.VARCHAR(length=225),
                                ...
                                'Comment_Text': sqlalchemy.types.VARCHAR(length=40000),
...
if Check_channel:
   # Create database connection
    engine = create_engine('mysql+mysqlconnector://root:123456@localhost/youtube_db', connect_args={'charset':'utf8'}, echo=False)
...
connect_for_question = pymysql.connect(host='localhost', user='root', password='123456', db='youtube_db')
